...
const Result = ({ value }: ResultProps) => {
  ...
  return (
    <aside className="column result">
      <div>
        <QRCode value={message} bgColor="#101c3d" fgColor="#FFFFFF" />
        <CopyToClipboard text={message} onCopy={copy}>
          <button className="is-primary">{copied ? 'Copied' : 'Copy'}</button>
        </CopyToClipboard>
        <footer>
          <h5>Scan the above QR code using Lisk Mobile</h5>
          <p>
            (You should be signed in with the account that you defined as the
            sender)
          </p>
          <nav>
            <a
